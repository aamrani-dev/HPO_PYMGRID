...
class MyClass(Trainable):
    def _setup(self, config):
        ...
        rl_algo_config = appo.DEFAULT_CONFIG.copy()
        ...
        rl_algo_config["vtrace"] = True
        rl_algo_config["clip_param"] = config["clip_param"]
        rl_algo_config["train_batch_size"] = config["train_batch_size"]
        rl_algo_config["lr"] = config["lr"]
        rl_algo_config["num_workers"] = 10
        rl_algo_config["num_sgd_iter"] = config["num_sgd_iter"]
        ...
        self.trainer = appo.APPOTrainer(env=MicroGridEnv, config=rl_algo_config)
